...
install: check_cargo
	cargo install --path .
	@echo "[+] passthecert-rs installed!"

uninstall:
	@cargo uninstall passthecert-rs
...
build_linux_x86_64:
	RUSTFLAGS="-C target-feature=+crt-static" cargo build --release --target x86_64-unknown-linux-gnu --no-default-features
...
build_macos_x86_64:
	@export PATH="/usr/local/bin/osxcross/target/bin:$PATH"
	RUSTFLAGS="-C target-feature=+crt-static" cargo build --release --target x86_64-apple-darwin --no-default-features
...
build_macos_aarch64:
	@export PATH="/usr/local/bin/osxcross/target/bin:$PATH"
	RUSTFLAGS="-C target-feature=+crt-static" cargo build --release --target aarch64-apple-darwin --no-default-features
...
arm_musl: check_rustup install_cross
	cross build --target arm-unknown-linux-musleabi --release --no-default-features
