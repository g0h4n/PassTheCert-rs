...
- [x] `read_object` (aka `read` / `dump` / `get_object`) : dump every attribute of an object :white_check_mark:
- [x] `rusthound_ce` : full [RustHound-CE](https://github.com/g0h4n/RustHound-CE) ([BloodHound-CE](https://github.com/SpecterOps/BloodHound)) collection over the certificate session :white_check_mark:
...
- [x] `remove_shadow_cred` / `list_shadow_cred` / `flush_shadow_cred` :white_check_mark:
